...
class AudioSinkSimulator(BaseFileSimulator):
    ...
    def __init__(
        ...
        # When True, incoming WAV data already contains a RIFF header
        # (e.g. CambAI streams a complete file). We write raw bytes
        # instead of using wave.writeframes() which would corrupt the
        # output by treating the header as PCM samples.
    ...
    def write(
        ...
        On the first WAV write, checks whether the data already
        contains a RIFF header (e.g. CambAI streams a complete WAV
        file). If so, switches to raw-byte passthrough to avoid
        ...
        if self._chunk_count % 10 == 0:
            print(f"Audio sink | recieved chunk: {self._chunk_count}")

    def close(self) -> None:
        """Close the audio file."""
        if self._file_opened is not None:
            self._file_opened.close()
            self._file_opened = None

    def is_open(self) -> bool:
        """Check if the audio file is open."""
        return self._file_opened is not None
